...
describe("Delete Item from a Cart - Positive API tests", () => {
  ...
  it.only("APITEST-01 As a user I want to delete item with quantity 1 from Cart", () => {
    // to check delete item by API deleteCartItemSF
    cy.get("@testdata").then((testdata) => {
      console.log(ItemId)
      const requestHeaders = {
        "Authorization": testdata.authorization,
  ...
  it("APITEST-02 As a user I want to delete item from Cart and check response JSON scheme", () => {
    // to check contract
  })
  
  it("APITEST-03 As a user I want to delete item with quantity 2 from Cart", () => {
    // to check desrease quantity button from 2 to 0 by API updateCartItem and deleteCartItemSF
  })
})